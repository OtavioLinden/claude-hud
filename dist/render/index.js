    ...
    for (let index = 0; index < elementOrder.length; index += 1) {
        ...
        if (combinablePair) {
            ...
            while (resultLine) {
                ...
                if (!nextNextElement || seen.has(nextNextElement))
                    break;
            ...
            if (resultLine) {
                ...
                if (canCombine) {
                    ...
                    const fallbackElements = [element, nextElement, ...Array.from(seen).filter(e => e !== element && e !== nextElement && elementOrder.indexOf(e) > index && elementOrder.indexOf(e) < index + chainOffset)];
